// Global variables
let timeLeft = 50 * 60; // seconds (50 minutes for focus)
let timerInterval;
let currentInterval = 'pomodoro'; // Default interval is focus
...
const longFocusIntervalBtn = document.getElementById('long-focus-interval-btn'); // Changed from longBreak to longFocus
...
pomodoroIntervalBtn.addEventListener('click', () => {
  ...
  timeLeft = 50 * 60; // 50 minutes for focus
  updateTimeLeftTextContent();
});

shortBreakIntervalBtn.addEventListener('click', () => {
  currentInterval = 'short-break';
  timeLeft = 10 * 60; // 10 minutes for short break
  updateTimeLeftTextContent();
});

longFocusIntervalBtn.addEventListener('click', () => {
  currentInterval = 'long-focus'; // Changed from longBreak to longFocus
  timeLeft = 120 * 60; // 120 minutes for long focus
  updateTimeLeftTextContent();
});
...
resetBtn.addEventListener('click', () => {
  ...
  if (currentInterval === 'pomodoro') {
    timeLeft = 50 * 60; // Reset to 50 minutes for focus
  } else if (currentInterval === 'short-break') {
    timeLeft = 10 * 60; // Reset to 10 minutes for short break
  } else {
    timeLeft = 120 * 60; // Reset to 120 minutes for long focus
  }
  updateTimeLeftTextContent();
...
function startTimer() {
  timerInterval = setInterval(() => {
    ...
    if (timeLeft === 0) {
      ...
      if (currentInterval === 'pomodoro') {
        timeLeft = 10 * 60; // Switch to 10 minutes for short break
        currentInterval = 'short-break';
        startTimer();
      } else if (currentInterval === 'short-break') {
        timeLeft = 120 * 60; // Switch to 120 minutes for long focus
        currentInterval = 'long-focus';
        startTimer();
      } else {
        timeLeft = 50 * 60; // Switch back to 50 minutes for focus
        currentInterval = 'pomodoro';
      }
    }
  }, 1000);
